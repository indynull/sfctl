...
class TestCli:
    def test_show_config_empty(self, monkeypatch, capsys):
        from sftui import cli

        monkeypatch.setattr("sys.argv", ["sftui", "--show-config"])
        cli.main()
        assert "(empty)" in capsys.readouterr().out

    def test_show_config_with_data(self, monkeypatch, capsys):
        from sftui import cli, config
    ...
    def test_clear_config_all(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("sys.argv", ["sftui", "--clear-config"])
        cli.main()
    ...
    def test_clear_config_keys(self, monkeypatch, capsys):
        ...
        config.save_config({"browser": "chrome", "theme": "dark"})
        monkeypatch.setattr("sys.argv", ["sftui", "--clear-config", "theme"])
        cli.main()
    ...
    def test_set_config(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("sys.argv", ["sftui", "--set", "theme", "monokai"])
        cli.main()
    ...
    def test_set_config_with_task(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "sys.argv",
            ["sftui", TASK_ID, "--set", "api_base", "https://test.example.com", "--fixture", str(FIXTURE_PATH)],
        )
        cli.main()
    ...
    def test_fixture_mode(self, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", ["sftui", "--fixture", str(FIXTURE_PATH)])
        cli.main()
    ...
    def test_fixture_verbose(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(StarfleetApp, "run", lambda self: None)
        monkeypatch.setattr("sys.argv", ["sftui", "--fixture", str(FIXTURE_PATH), "-v"])
        cli.main()
    ...
    def test_fixture_with_task_arg(self, monkeypatch):
        ...
        run_called = {}
        monkeypatch.setattr(StarfleetApp, "run", lambda self: run_called.update(task=self.task_arg))
        monkeypatch.setattr("sys.argv", ["sftui", "t-custom", "--fixture", str(FIXTURE_PATH)])
        cli.main()
    ...
    def test_fixture_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", ["sftui", "--fixture", "/nonexistent/file.json"])
    ...
    def test_no_task_errors(self, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", ["sftui"])
        with pytest.raises(SystemExit):
            cli.main()
    ...
    def test_task_with_cookies(self, monkeypatch):
        ...
        monkeypatch.setattr(StarfleetApp, "run", lambda self: None)
        monkeypatch.setattr("sys.argv", ["sftui", TASK_ID])
        cli.main()
    ...
    def test_task_verbose(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(api_mod, "fetch_data", lambda t, c: fixture_data)
        monkeypatch.setattr(StarfleetApp, "run", lambda self: None)
        monkeypatch.setattr("sys.argv", ["sftui", TASK_ID, "-v"])
        cli.main()
...
class TestMain:
    def test_main_calls_cli(self, monkeypatch):
        ...
        monkeypatch.setattr(sftui.cli, "main", fake_main)

        import importlib

        import sftui.__main__

        importlib.reload(sftui.__main__)
        assert called
